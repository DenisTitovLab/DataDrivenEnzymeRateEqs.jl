...
function optimal_rate_equation_selection(
    ...
    for num_params = start_num_params:end_num_params
        ...
        if forward_model_selection
            param_removal_codes = forward_selection_next_param_removal_codes(
        ...
        elseif !forward_model_selection
            param_removal_codes = reverse_selection_next_param_removal_codes(
                all_param_removal_codes,
                previous_param_removal_codes,
                num_params,
                param_names,
        ...
        #pmap over param_removal_codes for a given `num_params` return rescaled and nt_param_subset added
        # Fit equation with param_removal_codes parameters to training data
        results_array = pmap(
            param_removal_code -> train_rate_equation(
                ...
                param_names;
                n_iter = 20,
                nt_param_choice = param_removal_code,
            ),
            param_removal_codes,
        )
        ...
        previous_param_removal_codes = df_results.nt_param_choice

        #calculate test loss for top 10% subsets for each `num_params`
        #TODO: I guess don't use param_removal_code as train params are already converted using nt_param_choice
        test_loss = pmap(
...
"""
function param_subset_select(params, param_names, nt_param_choice)
    @assert length(params) == length(param_names)
    params_dict =
        Dict(param_name => params[i] for (i, param_name) in enumerate(param_names))

    for param_choice in keys(nt_param_choice)
        if startswith("L", string(param_choice)) && nt_param_choice[param_choice] == 1
            params_dict[:L] = 0.0
        elseif startswith("Vmax", string(param_choice)) &&
               nt_param_choice[param_choice] == 1
            params_dict[:Vmax_i] = params_dict[:Vmax_a]
        elseif startswith("Vmax", string(param_choice)) &&
               nt_param_choice[param_choice] == 2
            params_dict[:Vmax_i] = 0.0
        elseif startswith("K", string(param_choice)) && nt_param_choice[param_choice] == 1
            K_i = Symbol("K_i_" * string(param_choice)[3:end])
            K_a = Symbol("K_a_" * string(param_choice)[3:end])
            params_dict[K_i] = params_dict[K_a]
        elseif startswith("K", string(param_choice)) && nt_param_choice[param_choice] == 2
            K_a = Symbol("K_a_" * string(param_choice)[3:end])
            params_dict[K_a] = Inf
        elseif startswith("K", string(param_choice)) && nt_param_choice[param_choice] == 3
            K_i = Symbol("K_i_" * string(param_choice)[3:end])
            params_dict[K_i] = Inf
        elseif startswith("alpha", string(param_choice)) &&
               nt_param_choice[param_choice] == 0
...
"""
Calculate `param_removal_codes` with `num_params` including non-zero term combinations for codes (excluding alpha terms) in each `previous_param_removal_codes` that has `num_params-1`
"""
